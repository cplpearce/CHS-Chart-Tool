    ...
    for kap in chartIterator:
        ...
        try:
           ...
           ##   M E T A D A T A   U P D A T E   E N D S   ##
            ...
            errorCharts.append(kap[0])
            msgWarn("Issue working with chart {}".format(kap[0]))
            msg("This issue is rare but can be caused by a corrupted chart.")
            msg("Chart name logged for later examination...")
...
try:
    if len(errorCharts) != 0:
        ...
        pathErrors = os.path.join(directoryDecollared, "Charts-With-Errors.txt")
        with open(pathErrors, "w") as errorTF:
